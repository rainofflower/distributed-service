...
public class ZookeeperRegistry extends Registry {
    ...
    public void register(ProviderConfig providerConfig) {
        ...
        Set<String> methodSigSet = interfaceMethodMap.keySet();
        if(CommonUtils.isEmpty(methodSigSet)){
            return;
        }
        for(ServerConfig serverConfig : serverConfigs){
            ...
            for(String methodSig : methodSigSet){
                ...
                providerPathBuilder.append(RpcConstants.ZK_REGISTRY_ROOT_PATH)
                        ...
                        .append(CONTEXT_SEP).append(node);
                String providerPath = providerPathBuilder.toString();
                try {
                    zkClient.create().creatingParentContainersIfNeeded()
                            .withMode(CreateMode.EPHEMERAL)
                            .forPath(providerPath,node.getBytes("UTF-8"));
                }catch (Exception e){
    ...
    public List<ProviderGroup> subscribe(ConsumerConfig config) {
        ...
        for (Method method : methods) {
            ...
            StringBuilder consumerPathBuilder = new StringBuilder();
            consumerPathBuilder.append(RpcConstants.ZK_REGISTRY_ROOT_PATH)
                    .append(CONTEXT_SEP).append(interfaceName)
                    .append(CONTEXT_SEP).append(protocol)
                    .append(CONTEXT_SEP).append(methodSig)
                    .append(CONTEXT_SEP).append(version)
                    .append(CONTEXT_SEP).append(group)
                    .append(CONTEXT_SEP).append(PROVIDERS);
            String consumerPath = consumerPathBuilder.toString();
            try {
                PathChildrenCache childrenCache = new PathChildrenCache(zkClient, consumerPath, true);
